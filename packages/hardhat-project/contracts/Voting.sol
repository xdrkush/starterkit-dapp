...
contract Voting is Ownable {
    ...
    function _calcWinner() private {
        uint maxVoteCount = 0;
        for (uint i = 0; i < totalProposals.current(); i++) {
    ...
    function registerWhitelisted(address _addr) public onlyOwner onlyDuringStatus(WorkflowStatus.RegisteringVoters) {
    ...
    function newProposal(string memory _description) public onlyDuringStatus(WorkflowStatus.ProposalsRegistrationStarted) onlyWhitelisted(msg.sender) {
        uint proposalId = totalProposals.current();
    ...
    // | Mod : onlyDuringStatus(WorkflowStatus) - onlyWhitelisted(address)
    function voteOnProposal(uint proposalId) public onlyDuringStatus(WorkflowStatus.VotingSessionStarted) onlyWhitelisted(msg.sender) {
